...
let accessToken;
let tenantId;
let availableRoles = []; // Array to store available roles
...
test.describe.serial('Tenant Groups API Tests', () => {
    ...
    test.beforeAll(async ({ request }) => {
        try {
            ...
            // Fetch available roles to use in the "assign role" test
            const rolesResponse = await request.get(`${BASE_URL}/admin/tenants/${tenantId}/roles`, {
                headers: { 'Authorization': `Bearer ${accessToken}` }
            });

            if (rolesResponse.ok()) {
                availableRoles = await rolesResponse.json();
                console.log('Successfully fetched available roles.');
            } else {
    ...
    test('should assign a role to the group', async ({ request }) => {
        expect(newGroupId).toBeDefined();
        // Use a real role ID if available, otherwise default to a mock one.
        const roleIdToAssign = availableRoles.length > 0 ? availableRoles[0].id : 'mockRoleId12345';

        // The API expects a direct array, not an object containing the array.
        const assignmentData = [{ id: roleIdToAssign }];
    ...
    test('should delete the tenant group and user', async ({ request }) => {
        // Delete the user
        if (newUserId) {
